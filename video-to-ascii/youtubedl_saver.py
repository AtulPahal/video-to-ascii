...
def get_stream_url(url, max_height=480):
    """Extract a direct progressive video URL without downloading the full file.
    
    Returns (stream_url, fps, total_frames, duration).
    On error, stream_url is "error".
    """
    if not HAS_YTDL:
    ...
    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
        try:
            ...
            # Find the best progressive format (audio+video in one stream)
            ...
            if progressive:
                progressive.sort(key=lambda f: f.get('height', 0) or 0, reverse=True)
                stream_url = progressive[0]['url']
            else:
                # Fallback: use any format with a URL (worst case, no audio)
                for f in formats:
                    if f.get('url'):
                        stream_url = f['url']
                        break
                else:
                    print("Error: No streamable format found")
                    return "error", 0, 0, 0
        except Exception as e:
            print(f"Error getting stream URL: {e}")
            return "error", 0, 0, 0
    
    return stream_url, fps, total_frames, duration
