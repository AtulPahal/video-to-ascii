...
class ASCIIVideoPlayer:
    ...
    def __init__(self, args):
        ...
        self.format_height = getattr(args, 'format_height', 480)
    ...
    def load_video(self):
        ...
        if not re.match(
            ...
            return False
        # --- Stream YouTube URL (no full download) ---
        stream_url, self.framerate, self.total_frames, self.duration = ydls.get_stream_url(vid, max_height=self.format_height)
        if stream_url == "error":
            return False
        if self.framerate <= 0:
            self.framerate = 30.0
        self.audio_path = stream_url
        self.video_cap = cv2.VideoCapture(stream_url)
        if not self.video_cap.isOpened():
            print(f"{Colours.FAIL}Error: Could not open video stream{Colours.END}")
            return False
        print(f"{Colours.GREEN}Streaming: {self.total_frames} frames @ {self.framerate:.1f} fps, "
              f"{self.duration:.1f}s  (no full download){Colours.END}")
        return True
    ...
    def _start_audio(self):
        ...
        if not self.audio_player:
            return
        # For streaming URLs, only ffplay works; skip other players
        if not os.path.isfile(self.audio_path) and self.audio_player != "ffplay":
            return
